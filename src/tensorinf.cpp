#include "tensorinf.hpp"

float TENSORinf::letterbox( const cv::Mat& image, cv::Mat& out_image, const cv::Size& new_shape = cv::Size(640, 640), int stride = 32, const cv::Scalar& color = cv::Scalar(114, 114, 114), bool fixed_shape = false, bool scale_up = true) {
...
float* TENSORinf::blobFromImage(cv::Mat& img) {
...
void TENSORinf::draw_objects(const cv::Mat& img, float* Boxes, int* ClassIndexs, int* BboxNum) {
    for (int j = 0; j < BboxNum[0]; j++) {
      ...
      cv::putText( img, std::to_string(ClassIndexs[j]), cv::Point(rect.x, rect.y - 1), cv::FONT_HERSHEY_PLAIN, 1.2, cv::Scalar(0xFF, 0xFF, 0xFF), 2);
      cv::imwrite("result.jpg", img);
    }
}

TENSORinf::TENSORinf(char* model_path) {
...
void TENSORinf::Infer( int aWidth, int aHeight, int aChannel, unsigned char* aBytes, float* Boxes, int* ClassIndexs, int* BboxNum) {
