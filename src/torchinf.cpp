...
torch::Tensor TORCHInf::xyxy2xywh(const torch::Tensor& x) {
...
torch::Tensor TORCHInf::xywh2xyxy(const torch::Tensor& x) {
    auto y = torch::empty_like(x);
...
torch::Tensor TORCHInf::nms(const torch::Tensor& bboxes, const torch::Tensor& scores, float iou_threshold) {
    if (bboxes.numel() == 0)
...
torch::Tensor TORCHInf::non_max_suppression(torch::Tensor& prediction, float conf_thres, float iou_thres, int max_det) {
    ...
    for (int i = 0; i < bs; i++) {
        output.push_back(torch::zeros({0, 6 + nm}, this->device));
    }
...
torch::Tensor TORCHInf::clip_boxes(torch::Tensor& boxes, const std::vector<int>& shape) {
...
torch::Tensor TORCHInf::scale_boxes(const std::vector<int>& img1_shape, torch::Tensor& boxes, const std::vector<int>& img0_shape) {
